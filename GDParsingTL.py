...
SAMPLE_SPREADSHEET_ID = '1KT-PUCxAjDf3cuPVo3tcZxKyPzbelTc6MtAEyV5MOC8'
SAMPLE_RANGE_NAME_MONTH = config.month_TL
...
def getDataFromExcelTableTL():
    creds = None
    if os.path.exists('tokenTL.json'):
        creds = Credentials.from_authorized_user_file('tokenTL.json', SCOPES)
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            creds.refresh(Request())
        else:
            flow = InstalledAppFlow.from_client_secrets_file(
                'credentialsTL.json', SCOPES)
            creds = flow.run_local_server(port=0)
        with open('tokenTL.json', 'w') as token:
